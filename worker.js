...
async function api(request, env, path) {
  ...
  if (request.method === "GET" && /^\/api\/clans\/\d+$/.test(path)) {
    ...
    if (me) {
      const viewerMember = await env.DB.prepare(`
        SELECT role
        FROM members
      ...
      if (!viewerIsMember) {
        const pending = await env.DB.prepare(`
          ...
          LIMIT 1
        `).bind(id, me.id, clan.captain_id).first();

        viewerJoinPending = !!pending;
      }
    }
    ...
    return json(
      {
        ...
        viewer_join_pending: viewerJoinPending
      },
      200,
      headers
    );
  ...
  if (request.method === "POST" && path === "/api/clans") {
    ...
    const existingCode = await env.DB.prepare(`SELECT id FROM clans WHERE clan_code=? AND league=?`).bind(clanCode, league).first();
  ...
  /*
   * SOLICITUDES PARA UNIRSE A UN CLAN
   * Se reutiliza la tabla invites con status='join_pending'.
   * inviter_id = jugador que solicita
   * invitee_id = capitán del clan
   */
  const clanJoinMatch = path.match(/^\/api\/clans\/(\d+)\/join$/);
  if (request.method === "POST" && clanJoinMatch) {
    const clanId = Number(clanJoinMatch[1]);
    const clan = await env.DB.prepare(`
      SELECT id,name,clan_code,league,captain_id
      FROM clans
      WHERE id=?
      LIMIT 1
    `).bind(clanId).first();
    ...
    if (!clan) {
      return json({ error: "Clan no encontrado." }, 404, headers);
    }

    if (Number(clan.captain_id) === Number(me.id)) {
      return json({ error: "Ya eres el capitán de este clan." }, 400, headers);
    }

    const already = await getUserClan(env, me.id, Number(clan.league));
    if (already) {
      return json({ error: "Ya perteneces a un clan en este sector." }, 400, headers);
    }
    ...
    const pending = await env.DB.prepare(`
      ...
      WHERE clan_id=?
        ...
        AND status='join_pending'
      LIMIT 1
    `).bind(clanId, me.id, clan.captain_id).first();

    if (pending) {
      return json({ error: "Ya tienes una solicitud pendiente para este clan." }, 400, headers);
    }
  ...
  if (request.method === "GET" && path === "/api/clan-join-requests") {
    const rows = await env.DB.prepare(`
      SELECT
        i.id,
        i.clan_id,
        i.inviter_id AS user_id,
        i.created_at,
        u.username,
        u.psn_id,
        u.avatar_url,
  ...
  const joinRequestAction = path.match(/^\/api\/clan-join-requests\/(\d+)\/(accept|reject)$/);
  if (request.method === "POST" && joinRequestAction) {
    const requestId = Number(joinRequestAction[1]);
    const action = joinRequestAction[2];

    const joinRequest = await env.DB.prepare(`
      SELECT
        i.*,
        c.name AS clan_name,
        c.league,
      ...
      WHERE i.id=?
        ...
        AND c.captain_id=?
      LIMIT 1
    `).bind(requestId, me.id, me.id).first();

    if (!joinRequest) {
      return json({ error: "Solicitud no disponible." }, 404, headers);
    }
    ...
    const already = await getUserClan(
      env,
      joinRequest.inviter_id,
      Number(joinRequest.league)
    );
    ...
    if (already) {
      ...
      return json({
        error: "Ese jugador ya pertenece a un clan en este sector."
      }, 400, headers);
    }
  ...
  if (request.method === "POST" && acceptMatch) {
    ...
    const creatorMembers = await env.DB.prepare(`SELECT user_id FROM members WHERE clan_id=?`).bind(challenge.creator_clan_id).all();
    for (const member of creatorMembers.results) {
      await env.DB.prepare(`
        INSERT INTO notifications
        (user_id,title,message,type,challenge_id,target_type,target_id,is_read)
        VALUES (?,?,?,?,?,'challenge',?,0)
      `).bind(
        member.user_id,
        'Reto aceptado',
  ...
  /* ===================== COMPATIBILIDAD /api/me/clans ===================== */
  if (request.method === "GET" && path === "/api/me/clans") {
    const result=await env.DB.prepare(`
      SELECT c.*,m.role FROM clans c JOIN members m ON m.clan_id=c.id WHERE m.user_id=? ORDER BY c.league,c.id
    `).bind(me.id).all();
    return json(result.results,200,headers);
  }

  /* ===================== ALIAS ADMIN DEL INDEX ===================== */
  if (isAdmin(me) && request.method === "DELETE") {
    const mClan=path.match(/^\/api\/admin\/clans\/(\d+)$/);
    if(mClan){
      const id=Number(mClan[1]);
      await env.DB.batch([
        env.DB.prepare(`DELETE FROM members WHERE clan_id=?`).bind(id),
        env.DB.prepare(`DELETE FROM invites WHERE clan_id=?`).bind(id),
        env.DB.prepare(`DELETE FROM scores WHERE clan_id=?`).bind(id),
    ...
    if(mUser){
      ...
      if(id===me.id) return json({error:"No puedes borrar tu propia cuenta de administrador."},400,headers);
      await env.DB.batch([
        env.DB.prepare(`DELETE FROM notifications WHERE user_id=?`).bind(id),
        env.DB.prepare(`DELETE FROM invites WHERE inviter_id=? OR invitee_id=?`).bind(id,id),
        env.DB.prepare(`DELETE FROM members WHERE user_id=?`).bind(id),
  ...
  if (isAdmin(me) && request.method === "POST") {
    const mBlock=path.match(/^\/api\/admin\/users\/(\d+)\/block$/);
    if(mBlock){
      const id=Number(mBlock[1]);
      await env.DB.prepare(`UPDATE users SET is_blocked=1,blocked_until=NULL WHERE id=?`).bind(id).run();
      await env.DB.prepare(`DELETE FROM sessions WHERE user_id=?`).bind(id).run();
      return json({ok:true},200,headers);
    }
    const mUnblock=path.match(/^\/api\/admin\/users\/(\d+)\/unblock$/);
    if(mUnblock){
      await env.DB.prepare(`UPDATE users SET is_blocked=0,blocked_until=NULL WHERE id=?`).bind(Number(mUnblock[1])).run();
      return json({ok:true},200,headers);
    }
    const mPSN=path.match(/^\/api\/admin\/users\/(\d+)\/psn$/);
    if(mPSN){
      const data=await body(request); const psn=String(data.psn_id||"").trim().slice(0,32);
      await env.DB.prepare(`UPDATE users SET psn_id=?,psn_changed_at=? WHERE id=?`).bind(psn,Date.now(),Number(mPSN[1])).run();
      return json({ok:true},200,headers);
    }
    const mResult=path.match(/^\/api\/admin\/challenges\/(\d+)\/result$/);
    if(mResult){
      const data=await body(request); const challengeId=Number(mResult[1]); const winner=Number(data.winner_clan_id);
      const ch=await env.DB.prepare(`SELECT * FROM challenges WHERE id=?`).bind(challengeId).first();
      if(!ch) return json({error:"Reto no encontrado."},404,headers);
      if(![ch.creator_clan_id,ch.accepter_clan_id].includes(winner)) return json({error:"Ganador no válido."},400,headers);
      await env.DB.prepare(`UPDATE challenges SET status='completed',winner_clan_id=?,completed_at=CURRENT_TIMESTAMP WHERE id=?`).bind(winner,challengeId).run();
      return json({ok:true},200,headers);
    }
    const mLeague=path.match(/^\/api\/admin\/leagues\/(\d+)\/reset$/);
    if(mLeague){
      const league=Number(mLeague[1]); if(![1,2,3,4].includes(league)) return json({error:"Liga no válida."},400,headers);
      await env.DB.batch([
        env.DB.prepare(`UPDATE scores SET points=0,wins=0,losses=0,played=0 WHERE league=?`).bind(league),
        env.DB.prepare(`UPDATE challenges SET status='cancelled',cancel_reason='RESET_LEAGUE',cancelled_at=CURRENT_TIMESTAMP WHERE status IN ('open','accepted') AND team_size=?`).bind(league)
      ]);
  ...
  if (request.method === "GET" && path === "/api/my-challenges") {
    ...
    const result = await env.DB.prepare(`
      SELECT ch.*,
        ...
        creator.clan_code AS creator_code,
      ...
      JOIN clans creator ON creator.id=ch.creator_clan_id
      LEFT JOIN clans accepter ON accepter.id=ch.accepter_clan_id
      WHERE ch.creator_clan_id IN (SELECT clan_id FROM members WHERE user_id=?)
         OR ch.accepter_clan_id IN (SELECT clan_id FROM members WHERE user_id=?)
      ORDER BY ch.id DESC LIMIT 100
    `).bind(me.id, me.id).all();
    const rows=result.results.map(x=>({
      ...x,
      league:Number(x.league || x.team_size || 4),
      team_size:Number(x.team_size || x.league || 4),
      mode:x.one_vs_one_mode || (()=>{try{const p=JSON.parse(x.game_modes||"[]");return Array.isArray(p)&&p[0]?p[0]:"snd"}catch{return "snd"}})(),
      mine:true
    }));
    return json(rows,200,headers);
  }
  ...
  /* ===================== BORRAR NOTIFICACION ===================== */
  const deleteNotification = path.match(/^\/api\/notifications\/(\d+)$/);
  if (request.method === "DELETE" && deleteNotification) {
    await env.DB.prepare(`DELETE FROM notifications WHERE id=? AND user_id=?`)
      .bind(Number(deleteNotification[1]), me.id).run();
    return json({ok:true},200,headers);
  }

  /* ===================== COMUNIDAD ===================== */
  if (request.method === "GET" && path === "/api/community") {
    const info=await env.DB.prepare(`PRAGMA table_info(community_messages)`).all();
    const cols=info.results.map(x=>x.name);
    const select=cols.map(c=>`cm.${c}`).join(",");
    const result=await env.DB.prepare(`SELECT ${select} FROM community_messages cm ORDER BY cm.id DESC LIMIT 200`).all();
    return json(result.results,200,headers);
  }
  ...
  if (request.method === "POST" && path === "/api/community") {
    ...
    const values=[];
    const insertCols=[];
    const add=(name,value)=>{if(cols.includes(name)){insertCols.push(name);values.push(value)}};
    add("user_id",me.id); add("message",String(data.message||data.text||"").trim());
    if(cols.includes("league")) add("league", data.league==null?null:Number(data.league));
    if(cols.includes("created_at")) { /* default */ }
    if(!values.find((v,i)=>insertCols[i]==="message")) return json({error:"Escribe un mensaje."},400,headers);
    const marks=insertCols.map(()=>"?").join(",");
    const r=await env.DB.prepare(`INSERT INTO community_messages (${insertCols.join(",")}) VALUES (${marks})`).bind(...values).run();
    return json({ok:true,id:r.meta.last_row_id},200,headers);
  }
